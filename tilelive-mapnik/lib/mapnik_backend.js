...
MapnikSource.prototype._loadXML = function(callback) {
	var source = this;
	var pathname = this._uri.pathname;
	this._base = pathname ? path.resolve(path.dirname(pathname)) : process.cwd();
...
MapnikSource.prototype._createPool = function(xml, callback) {
	var source = this;
	if (!this._pool) {
		var poolSize = source._uri.query.poolSize || process.env.UV_THREADPOOL_SIZE || require('os').cpus().length;
		var poolOpts = { max: poolSize };
		var poolFactory = {
		...
		this._pool = new Pool(Object.assign({}, poolOpts, poolFactory));
	}
	callback(null);
};
...
MapnikSource.prototype._populateInfo = function(callback) {
	var source = this;
	var id = this._uri.pathname
		? path.basename(this._uri.pathname, path.extname(this._uri.pathname))
		: '<raw xml>'
